...
DECLARE
  ...
  v_is_last BOOLEAN; v_tenure INT; v_blocks INT;
BEGIN
  ...
  FOR v_emi IN
    SELECT es.id, es.customer_id, es.emi_no, es.due_date, es.fine_amount, es.fine_waived, es.fine_paid_amount, c.emi_tenure
  ...
  LOOP
    ...
    ELSE
      ...
      FOR b IN 1..v_blocks LOOP
        v_calc := v_calc + v_base;
        IF v_days > (b * 30) THEN
          v_calc := v_calc + ((v_days - b * 30) / 7) * v_weekly;
        END IF;
      END LOOP;
    END IF;
    ...
    IF v_calc != v_old THEN
      ...
      VALUES (v_emi.customer_id, v_emi.id, v_emi.emi_no,
        ...
        v_days || 'd overdue' || CASE WHEN v_is_last THEN ' (LAST EMI)' ELSE '' END || '. Fine: ' || v_old || '→' || v_calc);
      v_count := v_count + 1;
    END IF;
  END LOOP;
