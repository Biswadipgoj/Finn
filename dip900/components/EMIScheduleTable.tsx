...
interface Props {
  ...
  onRefresh?: () => void;
  defaultFineAmount?: number; // from fine_settings, default 450
}
                ...
                <tr key={emi.id} className={isOverdue ? 'bg-danger-light/30' : isNext ? 'bg-brand-50/50' : ''}>
                  ...
                  <td>
                    {editing ? (
                      ...
                      const isEmiOverdue = emi.status === 'UNPAID' && new Date(emi.due_date) < new Date();
                      // If overdue, fine_amount=0, and not waived → show default fine
                      const displayFine = (emi.fine_amount ?? 0) > 0
                        ? emi.fine_amount
                        : (isEmiOverdue && !emi.fine_waived ? defaultFineAmount : 0);
                      return displayFine > 0
                        ? <span className={`num text-xs font-semibold ${emi.fine_waived ? 'line-through text-ink-muted' : 'text-danger'}`}>{fmt(displayFine)}</span>
