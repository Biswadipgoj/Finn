...
export async function PATCH(
  ...
  if (emiIds.length > 0) {
    if (status === 'APPROVED' && before.status !== 'APPROVED') {
      ...
      // Record fine as PAID (do NOT zero fine_amount — it must persist)
      const fineCollected = Number(fine_amount ?? before.fine_amount) || 0;
      if (fineCollected > 0) {
        const lowestEmiNo = Math.min(...(items || []).map(i => i.emi_no));
        await svc.from('emi_schedule')
          .update({ fine_paid_amount: fineCollected, fine_paid_at: new Date().toISOString() })
        ...
        await svc.from('fine_history').insert({
          ...
          emi_no: lowestEmiNo,
          fine_type: 'PAID',
          fine_amount: fineCollected,
          cumulative_fine: fineCollected,
