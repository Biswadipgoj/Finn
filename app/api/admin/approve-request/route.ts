...
export async function POST(req: NextRequest) {
  ...
  const { error: emiErr } = await svc
    ...
    .update({
      ...
      paid_at:              now,
      mode:                 request.mode,
      utr:                  (request as Record<string, unknown>).utr as string || null,
      approved_by:          user.id,
  ...
  if ((request.fine_amount as number) > 0 && lowestEmiNo !== null) {
    ...
    await svc.from('fine_history').insert({ customer_id: request.customer_id, emi_schedule_id: emiIds[0] || null, emi_no: lowestEmiNo, fine_type: 'PAID', fine_amount: request.fine_amount, cumulative_fine: request.fine_amount, fine_date: now.split('T')[0], reason: 'Approved fine via ' + request.mode }).catch(() => {});
  }
