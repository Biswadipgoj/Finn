...
export async function POST(req: NextRequest) {
  try {
    ...
    if (fetchErr || !request) return NextResponse.json({ ok: false, error: 'Payment request not found' }, { status: 404 });
    if (request.status === 'APPROVED') return NextResponse.json({ success: true, already_approved: true, request_id });
    if (request.status !== 'PENDING') return NextResponse.json({ ok: false, error: `Cannot approve: status is ${request.status}` }, { status: 400 });
    ...
    if (auditErr) console.warn('approve-request audit_log insert failed', auditErr.message);
