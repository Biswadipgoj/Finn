...
export async function POST(req: NextRequest) {
  ...
  if ((request.fine_amount ?? 0) > 0) {
    ...
    const { error: fineErr } = await serviceClient
      .from('emi_schedule')
      .update({ fine_paid_amount: request.fine_amount, fine_paid_at: now })
      .eq('customer_id', request.customer_id)
      .eq('emi_no', lowestEmiNo);

    if (fineErr) console.error('Failed:', fineErr);
    await serviceClient.from('fine_history').insert({
      customer_id: request.customer_id, emi_schedule_id: items[0]?.emi_schedule_id || null,
      emi_no: lowestEmiNo, fine_type: 'PAID', fine_amount: request.fine_amount,
      cumulative_fine: request.fine_amount, fine_date: now.split('T')[0],
      reason: 'Approved fine ' + request.fine_amount + ' via ' + request.mode,
    }).catch(() => {});
  }
