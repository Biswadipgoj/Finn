...
export async function POST(req: NextRequest) {
  ...
  if (customer.retailer_id !== retailer.id) {
  ...
  const { data: request, error: re } = await svc.from('payment_requests').insert({
    customer_id, retailer_id: retailer.id, submitted_by: user.id, status: 'PENDING_APPROVAL', mode,
  ...
  if (!noEmi && emi_ids?.length) {
    ...
    if (ie) {
      ...
      return NextResponse.json({ error: 'Failed to record items' }, { status: 500 });
    }
    const { data: markedRows, error: markErr } = await svc
      .from('emi_schedule')
      .update({ status: 'PENDING_APPROVAL' })
      .in('id', emi_ids)
      .in('status', ['UNPAID', 'PARTIALLY_PAID'])
      .select('id');
    if (markErr || (markedRows || []).length !== emi_ids.length) {
      await svc.from('payment_requests').delete().eq('id', request.id);
      return NextResponse.json({ error: markErr?.message || 'One or more selected EMIs were collected by another request.' }, { status: 409 });
    }
  }
