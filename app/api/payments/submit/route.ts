...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!retail_pin?.trim()) return NextResponse.json({ ok: false, error: 'Retailer PIN required' }, { status: 400 });
    if (mode === 'UPI' && !utr?.trim()) return NextResponse.json({ ok: false, error: 'UTR required for UPI' }, { status: 400 });
    ...
    if (!retailer?.is_active) return NextResponse.json({ ok: false, error: 'Retailer inactive' }, { status: 403 });
    if (retailer.retail_pin !== retail_pin) return NextResponse.json({ ok: false, error: 'Incorrect PIN' }, { status: 401 });
    ...
    if (!noEmi && emi_ids?.length) {
      ...
      if ((ch || []).some(e => e.status === 'APPROVED' || e.status === 'PENDING_APPROVAL')) {
        return NextResponse.json({ ok: false, error: 'EMI already pending or fully paid' }, { status: 409 });
      }
    }
    ...
    if (re || !request) return NextResponse.json({ ok: false, error: re?.message || 'Failed' }, { status: 500 });
    ...
    if (!noEmi && emi_ids?.length) {
      ...
      if (ie) { await svc.from('payment_requests').delete().eq('id', request.id); return NextResponse.json({ ok: false, error: 'Failed to record items' }, { status: 500 }); }
