...
.select('id, message, image_url, expires_at, sender_name, sender_role')
  ...
  const hasCollectableEmis = selectedCustomer
    ? customerEmis.some(e => e.status === 'UNPAID' || e.status === 'PARTIALLY_PAID')
    : false;
  ...
  return (
    <div className="min-h-screen page-bg">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-44 md:pb-24">
        {/* Welcome Banner */}
        ...
        {showUpcoming && upcomingEmis !== null && (
          <div className="card overflow-hidden mb-6 animate-fade-in responsive-table-card">
                  ...
                  {upcomingEmis.map(e => {
                    ...
                    return (
                      <tr key={e.id}>
                        ...
                        <td data-label="EMI #"><span className="font-num">#{e.emi_no}</span></td>
                        ...
                        <td data-label="Amount"><span className="font-num text-brand-600">{fmt(e.amount)}</span></td>
                        <td data-label="Mobile"><span className="font-num text-ink-muted">{e.mobile}</span></td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <div className="animate-fade-in">
            ...
            {myRequests.length > 0 && (
              <div>
                ...
                <div className="card overflow-hidden responsive-table-card">
                  <table className="data-table text-xs sm:text-sm">
                    <thead>
                      ...
                      {myRequests.map(r => {
                        ...
                        return (
                          <tr key={r.id}>
                            ...
                            <td data-label="Amount"><span className="font-num font-semibold">{fmt(r.total_amount)}</span></td>
                            <td data-label="Mode"><span className={`text-xs font-semibold ${r.mode === 'UPI' ? 'text-info' : 'text-success'}`}>{r.mode}</span></td>
                            ...
                            <td data-label="Date" className="text-xs text-ink-muted">{format(new Date(r.created_at), 'd MMM, h:mm a')}</td>
        ...
        {searchResults !== null && searchResults.length > 1 && !selectedCustomer && (
          <div className="card overflow-hidden animate-fade-in responsive-table-card">
              ...
              <tbody>
                {searchResults.map(c => (
                  <tr key={c.id} onClick={() => selectCustomer(c)} className="cursor-pointer">
                    ...
                    <td data-label="IMEI"><span className="font-num text-xs">{c.imei}</span></td>
                    <td data-label="Mobile"><span className="font-num">{c.mobile}</span></td>
                    <td data-label="Status">
                      {c.status === 'RUNNING' ? <span className="badge-running">Running</span> : <span className="badge-complete">Complete</span>}
                    </td>
                    <td data-label="EMI"><span className="font-num text-brand-600">{fmt(c.emi_amount)}</span></td>
                    <td data-label="Open">
      ...
      {selectedCustomer?.status === 'RUNNING' && (
        <div className="md:hidden fixed left-0 right-0 bottom-16 z-40 bg-white/95 backdrop-blur border-t border-surface-4 p-3 safe-bottom shadow-[0_-8px_24px_rgba(15,23,42,0.08)] no-print">
          <div className="max-w-lg mx-auto flex items-center gap-3">
            <div className="flex-1 min-w-0">
              <p className="text-[11px] font-semibold text-ink-muted uppercase tracking-wide">Next Action</p>
              <p className="text-sm font-bold text-ink truncate">
                {hasCollectableEmis ? `Collect EMI #${breakdown?.next_emi_no ?? ''}` : 'All EMIs Paid'}
              </p>
            </div>
            <button
              onClick={() => setShowPaymentModal(true)}
              disabled={!hasCollectableEmis}
              className="btn-primary px-4 py-3 min-w-[148px] shadow-lg shadow-brand-500/20"
            >
              {hasCollectableEmis ? '💳 Collect' : '✓ Paid'}
            </button>
          </div>
        </div>
      )}
